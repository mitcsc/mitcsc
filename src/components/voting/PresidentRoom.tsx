...
export default function PresidentRoom() {
  ...
  return <div className="voter-room"><section className="voter-panel voter-join"><Image className="voter-join-logo" src="/img/logo/logo.png" alt="MIT CSC" width={144} height={144} priority/>{error && <div className="voter-alert" role="alert">{error}</div>}{login || overview ? <form onSubmit={submit}>{login ? <input aria-label="Code" placeholder="Code" type="password" autoComplete="current-password" required value={password} onChange={e=>setPassword(e.target.value)}/> : <><input aria-label="Name" placeholder="Name" required maxLength={100} value={name} onChange={e=>setName(e.target.value)}/><input aria-label="Code" placeholder="Code" type="password" autoComplete="new-password" required maxLength={500} value={voterPassword} onChange={e=>setVoterPassword(e.target.value)}/><input aria-label="Results spreadsheet link" placeholder="Results spreadsheet link" type="url" required value={sheetUrl} onChange={e=>setSheetUrl(e.target.value)}/><p className="president-help">Use a new spreadsheet shared as Editor with {overview?.serviceAccount || "the service account"}.</p></>}<button className="voter-primary" disabled={busy || (login ? !password : !name.trim() || !voterPassword || !sheetUrl.trim())}>{busy ? "Connecting…" : login ? "Continue" : "Create election"}</button></form> : <p role="status">Connecting…</p>}</section></div>;
}
