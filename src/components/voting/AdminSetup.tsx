...
export default function AdminSetup({ candidates: initialCandidates, criteria: initialCriteria, busy, onSave }: Props) {
  ...
  const save = async () => {
    ...
    if (await onSave(candidates, criteria)) { setDirty(false); setMessage("Setup saved. The listed candidate order will be used unless you shuffle it."); }
  };
  return <section className="voting-admin-card">
    <div className="voting-admin-heading"><div><p className="voting-admin-kicker">Before you begin</p><h2>Build your ballot</h2></div><button type="button" onClick={() => setPreview(!preview)}>{preview ? "Edit setup" : "Preview ballot"}</button></div>
    {preview ? <div className="voting-admin-preview"><p className="voting-admin-kicker">Ballot preview · changes are not submitted</p><h3>{candidates[0]?.name || "Candidate name"}</h3>{criteria.map(c => <fieldset key={c.id}><legend>{c.label || "Untitled criterion"}{c.required ? " *" : ""}</legend><p>{c.description}</p><div className="voting-admin-ratings">{Array.from({length: Math.max(0, Math.min(11, c.max - c.min + 1))}, (_, i) => c.min + i).map(n => <label key={n}><input type="radio" name={`preview-${c.id}`} value={n}/><span>{n}</span></label>)}</div>{!c.required && <label><input type="radio" name={`preview-${c.id}`} value=""/> Not enough information</label>}</fieldset>)}<p className="voting-admin-muted">Candidate context stays hidden until you reveal it during deliberation.</p></div> : <>
      <fieldset disabled={busy} className="voting-admin-editor"><legend>Candidates <span>{candidates.length}</span></legend><p className="voting-admin-muted">Listed order is the default. Use the arrows to reorder or shuffle the saved queue below.</p>
        {candidates.map((c, i) => <div className="voting-admin-edit-row" key={c.id}><div className="voting-admin-number">{i + 1}</div><div className="voting-admin-fields"><label>Name<input value={c.name} maxLength={120} onChange={e => updateCandidates(candidates.map(x => x.id === c.id ? {...x, name: e.target.value} : x))}/></label><label>Context <span className="voting-admin-muted">optional; revealed separately</span><textarea value={c.context} rows={2} maxLength={4000} onChange={e => updateCandidates(candidates.map(x => x.id === c.id ? {...x, context: e.target.value} : x))}/></label></div><div className="voting-admin-row-actions"><button aria-label={`Move ${c.name || "candidate"} up`} disabled={i === 0} onClick={() => move(i, -1)}>↑</button><button aria-label={`Move ${c.name || "candidate"} down`} disabled={i === candidates.length - 1} onClick={() => move(i, 1)}>↓</button><button aria-label={`Remove ${c.name || "candidate"}`} onClick={() => updateCandidates(candidates.filter(x => x.id !== c.id))}>Remove</button></div></div>)}
