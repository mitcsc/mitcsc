...
const phases: { phase: VotingPhase; label: string; description: string }[] = [
  ...
  {phase: "revision", label: "Allow revisions", description: "Voters can revise their ratings locally."},
...
export default function DeliberationsAdminPage() {
  ...
  const changePhase = (phase: VotingPhase) => {
    if (phase === "locked" && !window.confirm(`Close ${state?.currentCandidate?.name}? ${state?.submittedCount ?? 0} final ballots are saved. Initial drafts remain only in voters’ browsers, so confirm everyone has submitted. This candidate cannot be reopened in this version.`)) return;
    void act({action: "setPhase", phase});
  };
  const selectCandidate = (id: string) => {
    if (state?.currentCandidate && state.phase !== "locked" && state.phase !== "waiting" && !window.confirm("Switch candidates? Unsaved browser drafts are not submitted automatically. Confirm voters have submitted before continuing.")) return;
