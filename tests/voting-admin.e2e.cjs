...
(async () => {
  ...
  let polls = 0;
  let state = { sessionId: 'test-election', active: true, phase: 'waiting', ballotVersion: '', currentCandidate: null, criteria: [], contextVisible: false, submittedCount: 0, voter: null, isAdmin: true, participants:[{id:'v1',name:'Voter One',submitted:false,initialSubmitted:true},{id:'v2',name:'Voter Two',submitted:true,initialSubmitted:true}], initialized: false, candidates: [], spreadsheetUrl: 'https://docs.google.com/spreadsheets/d/test/edit' };
  const actions = [];
