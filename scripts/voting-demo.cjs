...
const landing=`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>CSC voting demo</title><style>body{background:#0c0c0c;color:#f4f4f4;font:17px/1.6 system-ui;max-width:700px;margin:60px auto;padding:24px}h1{font-size:38px;line-height:1.15}a{color:#ad292f}strong{font-weight:650}.button{display:inline-block;background:#b62f36;color:white;text-decoration:none;padding:12px 20px;border-radius:7px;margin:10px 0}li{margin:12px 0}code{background:#292929;padding:2px 6px}</style></head><body><p>MIT CSC · LOCAL DEMO</p><h1>Try the deliberation room.</h1><p>This demo uses sample candidates and keeps final votes in memory. It never connects to Google Sheets.</p><ol><li>Open the voting page, enter your name, and use password <code>demo</code>. The first person who joins becomes admin.</li><li>Select a candidate and open initial ratings.</li><li>Open the voter page in a private browser window to test as a voter. Admins cannot vote.</li><li>Use admin controls to move through discussion, voting, and closing.</li></ol><a class="button" href="/vote">Open voting →</a><p><a href="/vote" target="_blank">Open voter page in another tab</a></p><p>Restarting this demo clears server data. Your browser drafts remain saved.</p></body></html>`;
(async()=>{
