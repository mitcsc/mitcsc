...
const {signIdentity}=require('../src/lib/voting/security.ts');
const output='/private/tmp/voting-redis-live-result.json';
...
async function main(){
 ...
 const historyResponse=await google.request({url:`https://sheets.googleapis.com/v4/spreadsheets/${settingsId}/values/'Session History'!A:F`});
 const history=(historyResponse.data.values||[]).slice(1).filter(r=>r[0]===config.session_id&&r[1]===sheetId);
 const first=history[0];
 if(!first || first[4]!=='Load Test Admin')throw Error('The original rehearsal admin does not match; stopping');
 const identityFromRow=(row,role)=>({id:row[3],claimId:row[2],name:row[4],role,sessionId:config.session_id,sheetId,exp:Date.now()+86400000});
 const owner=agent('admin');owner.cookies.set('csc_voting',signIdentity(identityFromRow(first,'admin')));
 ...
 if(state.votingStarted)throw Error('This test election has already started; refusing to reset it.');
 ...
 await Promise.all(voters.map(async(who,i)=>{await sleep(i*30);const old=history.slice(1).find(r=>r[4]==='Test Voter '+String(i+1).padStart(2,'0'));if(old)who.cookies.set('csc_voting_device',signIdentity(identityFromRow(old,'voter'),{purpose:'device',ttlMs:86400000}));await api(who,'join',{name:'Test Voter '+String(i+1).padStart(2,'0'),password:config.session_password,joinId:randomUUID()},true);const s=await api(who,'state');if(s.isAdmin)throw Error('Unexpected second admin');poll(who,4000);}));
 progress('All 30 voters joined; starting immediately');
 ...
 await stage('initial',candidates[0].id);await initial(0);
 await stage('deliberation');progress('150-second discussion started, crossing a settings refresh');
 for(let i=0;i<3;i++){await sleep(50000);progress('Discussion elapsed',{seconds:(i+1)*50});}
 ...
 await Promise.all(voters.map(async(v,i)=>{await sleep(i*0);await api(v,'submit',ballots[i],true);}));progress('All 30 final submissions acknowledged');
 await stage('locked');await stage('initial',candidates[1].id);await initial(0);
 await stage('locked');
 ...
 const report={startedAt:events[0].startedAt,finishedAt:new Date().toISOString(),origin:base,revision:'local-redis-integration',voters:30,valid,responseRows:responseRows.length,uniqueBallots:uniqueBallots.size,initialReceipts:receipts.length,httpErrors:events.filter(e=>e.status===null||e.status>=400),events};
